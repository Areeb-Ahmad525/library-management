...
class CLI:
    ...
    def run(self) -> None:
        ...
        while True:
            choice = self._display_menu(
                ...
                ["Book Management", "Member Management", "Loan Management", "Exit"],
            )
    ...
    def _list_books(self) -> None:
        ...
        self._print_rows(
            empty_message="No books found.",
            headers=["ID", "Title", "Author"],
            rows=[(book.id, book.title, book.author) for book in books],
        )
    ...
    def _list_members(self) -> None:
        ...
        self._print_rows(
            ...
            rows=[(member.id, member.name, member.email) for member in members],
        )
    ...
    def _view_active_loans(self) -> None:
        ...
        self._print_rows(
            empty_message="No active loans found.",
            headers=["ID", "Book ID", "Member ID"],
            rows=[(loan.id, loan.book_id, loan.member_id) for loan in loans],
        )
    ...
    def _view_member_loans(self) -> None:
        ...
        self._print_rows(
            empty_message="No loans found for this member.",
            headers=["ID", "Book ID", "Member ID"],
            rows=[(loan.id, loan.book_id, loan.member_id) for loan in loans],
        )

    def _display_menu(self, title: str, options: list[str]) -> int:
        """Render a numbered menu and return the selected option."""
        self._print_header(title)
        for index, option in enumerate(options, start=1):
            print(f"{index}. {option}")
        return self._prompt_choice("Select an option", 1, len(options))
